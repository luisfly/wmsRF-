...
namespace TJ.WMS.RF.Service
{
    ...
    /// 采购收货
    /// </summary>
    public class DcAccept : RFBase
    {
        public DcAccept(string user_id, string user_name, string token)
        {
        ...
        {
            //ValidateUser();
            QueryObject query = GetQueryObject("RF_SelAcptGoodsInfo");
            if (query == null)
        ...
        {
            //ValidateUser();
            QueryObject query = GetQueryObject("RF_SelCSAcptGoodsInf");
            if (query == null)
        ...
        {
            //ValidateUser();
            QueryObject query = GetQueryObject("RF_SelCSOrderStore");
            if (query == null)
                throw new RFException("查询对象[RF_SelCSOrderStore]不存在");
            DataSet ds = Bss_Helper.GetDataSet_SQL(query.Sql, paras.Values.ToArray());
            return ds.Tables[0];
        }
