...
{
    public class ContainerAcptService:RFBase
    {
        public ContainerAcptService(string user_id, string user_name, string token)
        {
            SetParameter("Operator", user_id);
            SetParameter("OperName", user_name);

            this.Token = token;
            this.BusinessID = "RF_ContainerAcpt";

            ValidateUser();
        }
        public void ValidateContainerNO()
        {
            ValidateUser();
